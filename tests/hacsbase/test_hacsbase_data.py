...
async def test_recreated_repository_id_persists_across_restart(
    hacs, repository_integration, caplog
):
    data = HacsData(hacs)
    hacs.repositories = HacsRepositories()
    repository_full_name = repository_integration.data.full_name
    ...
    async def _load_stale_repository(hass, key):
        if key == "repositories":
    ...
    with patch(
        "custom_components.hacs.utils.data.async_load_from_store",
        side_effect=_load_stale_repository,
    ):
        assert await data.restore()

    stale_repository = hacs.repositories.get_by_id(REPOSITORY_ID_STALE)
    assert stale_repository is not None
    ...
    assert hacs.repositories.get_by_id(REPOSITORY_ID_STALE) is None
    assert hacs.repositories.get_by_id(REPOSITORY_ID_CURRENT) is stale_repository
    assert hacs.repositories.list_all == [stale_repository]
    assert hacs.repositories.is_default(REPOSITORY_ID_CURRENT)
    assert stale_repository.data.installed is True
    assert "duplicate IDs" not in caplog.text

    with patch(
        "custom_components.hacs.utils.data.async_save_to_store",
        new_callable=AsyncMock,
    ) as async_save_to_store:
        await data.async_write()

    persisted_repositories = next(
        call.args[2]
        for call in async_save_to_store.await_args_list
        if call.args[1] == "repositories"
    )
    assert set(persisted_repositories) == {REPOSITORY_ID_CURRENT}
    assert REPOSITORY_ID_STALE not in persisted_repositories
    assert persisted_repositories[REPOSITORY_ID_CURRENT]["installed"] is True
    assert persisted_repositories[REPOSITORY_ID_CURRENT]["show_beta"] is True
    assert (
        persisted_repositories[REPOSITORY_ID_CURRENT]["version_installed"] == "1.0.0"
    )

    hacs.repositories = HacsRepositories()
    restarted_data = HacsData(hacs)

    async def _load_persisted_repository(hass, key):
        if key == "repositories":
    ...
    with patch(
        "custom_components.hacs.utils.data.async_load_from_store",
        side_effect=_load_persisted_repository,
    ):
        assert await restarted_data.restore()

    restarted_repository = hacs.repositories.get_by_id(REPOSITORY_ID_CURRENT)
    assert restarted_repository is not None
    assert hacs.repositories.get_by_id(REPOSITORY_ID_STALE) is None
    assert hacs.repositories.list_all == [restarted_repository]
    assert restarted_repository.data.installed is True
    assert restarted_repository.data.show_beta is True
    assert restarted_repository.data.installed_version == "1.0.0"

    with patch.object(hacs.data_client, "get_data", return_value=category_data):
        await hacs.async_get_category_repositories_experimental(HacsCategory.INTEGRATION)

    assert hacs.repositories.get_by_id(REPOSITORY_ID_CURRENT) is restarted_repository
    assert hacs.repositories.get_by_id(REPOSITORY_ID_STALE) is None
    assert hacs.repositories.list_all == [restarted_repository]
    assert hacs.repositories.is_default(REPOSITORY_ID_CURRENT)
    assert "duplicate IDs" not in caplog.text
