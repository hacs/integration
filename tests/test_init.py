...
async def test_async_remove_config_entry_device_finds_first_valid_identifier(hass: HomeAssistant, hacs: HacsBase):
    """Test that function uses a valid HACS identifier when multiple are found."""
    hacs.repositories.is_downloaded = MagicMock(return_value=False)
    config_entry = MagicMock(spec=ConfigEntry)
    ...
    device_entry.identifiers = {
        ("other_domain", "other_id"),
        (DOMAIN, "first_repo"),  # Valid HACS identifier
        (DOMAIN, "second_repo"),  # Another valid HACS identifier
    }

    # Test successful removal using one of the valid identifiers
    result = await async_remove_config_entry_device(hass, config_entry, device_entry)
    assert result is True
    # Should use one of the valid identifiers (order not guaranteed with sets)
    hacs.repositories.is_downloaded.assert_called_once()
    called_args = hacs.repositories.is_downloaded.call_args[0]
    assert called_args[0] in ["first_repo", "second_repo"]
